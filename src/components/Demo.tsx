...
`.trim();
      ...
      // сохраняем сырой ответ для дебага/отображения
      setRawResponse(data);

      // проверяем, что структура такая, как мы ждём
      if (!data || !data.original || !Array.isArray(data.alternatives)) {
        setAiError("AI вернул данные в неожиданном формате");
        return;
      }
      ...
      const original: ResultOriginal = {
        ...
        price: String(data.original.price ?? ""),
      ...
      let alternatives: ResultAlternative[] = data.alternatives.map((a, i) => {
        ...
        if (a.savings !== undefined && a.savingsPercent !== undefined) {
          ...
          savingsPercentStr = `${percent.toFixed(0)}%`;
        }
        ...
        return {
          ...
          savingsPercent:
              a.savingsPercent !== undefined
                  ? String(a.savingsPercent)
                  : savingsPercentStr,
          isRecommended: a.isRecommended,
        };
      });

      // если AI не пометил лучший вариант — выберем по наименьшей цене
      if (
          alternatives.length > 0 &&
          !alternatives.some((a) => a.isRecommended)
      ) {
      ...
      setAiError("Не удалось связаться с AI-сервером");
    } finally {
