...
`.trim();
            ...
            // --- 2. Карточка "Ваш выбор" — делаем её про запрос пользователя ---
            const aiOriginalObj: AiOriginalObject | null =
                typeof data.original === "string" ? null : data.original || null;

            const userLines: string[] = [];

            if (searchQuery.trim()) {
                userLines.push(`Вы хотите: ${searchQuery.trim()}`);
            }

            if (budget) {
                userLines.push(`Бюджет: ${budget} €`);
            }

            if (aiOriginalObj?.name) {
                userLines.push(`Место, которое вы выбрали: ${aiOriginalObj.name}`);
            }

            userLines.push(
                "Но вы можете заказать и в других местах — AI подобрал варианты ниже 👇"
            );

            const original: ResultOriginal = {
                name: "Ваш запрос",
                items: userLines.join("\n"),
                price: budget || String(aiOriginalObj?.price ?? "0"),
                location: aiOriginalObj?.location || "—",
                deliveryTime: aiOriginalObj?.deliveryTime || "—",
            };
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                <div
                                    className="size-10 rounded-xl bg-gradient-to-br from-blue-500 to-purple-500 flex items-center justify-center">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {cart.length > 0 && (
                                ...
                                >
                                    ...
                                    <Badge
                                        className="absolute -top-1 -right-1 size-5 flex items-center justify-center bg-green-500 text-white border-0 p-0">
                                        {cart.length}
                                    </Badge>
                                </button>
                            )}
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <div className="relative">
                                                <Euro
                                                    className="absolute left-4 top-1/2 -translate-y-1/2 size-5 text-zinc-500"/>
                                                <input
                                                    type="number"
                ...
                {currentStep === "analyzing" && (
                    <div className="space-y-8 animate-in fade-in duration-500">
                        <div className="text-center space-y-6">
                            <div
                                className="inline-flex items-center justify-center size-20 rounded-full bg-blue-500/10 border-2 border-blue-500/30 animate-pulse">
    ...
    <span className="text-2xl text-zinc-100">
      {results.original.price}€
                                    ...
                                    <div
                                        className="flex items-center gap-4 text-sm text-zinc-500 pt-4 border-t border-zinc-800">
                            ...
                            <span className="text-sm text-zinc-500">
                              Фильтры:
                                  ...
                                  -{alt.savings} €
                                        ...
                                        {/* Summary */}
                                        <Card
                                            className="bg-gradient-to-br from-green-950/40 to-green-900/20 border-green-800/40 p-6">
                                            <div className="flex items-center justify-between">
                                                <div>
            ...
            {showCartNotification && (
                <div className="fixed bottom-4 right-4 z-50">
                    <Card
                        className="bg-green-500/10 border border-green-500/30 p-4 rounded-xl shadow-lg shadow-green-500/20">
                        <div className="flex items-center gap-3">
